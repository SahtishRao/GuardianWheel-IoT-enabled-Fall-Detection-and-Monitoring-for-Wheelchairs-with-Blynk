...
const unsigned long trigger2Timeout = 500; // Timeout for Trigger 2 detection (2 seconds)
...
void loop() {
  ...
  if (triggerCheck == 3) {
    ...
    if (totalAccel > accelThreshold) {
      ...
      trigger2Time = millis();
      triggerCheck++; // Proceed to Trigger  despite timeout
    }
  }
  ...
  if (triggerCheck == 4) {
    ...
    if (tiltAngle > tiltThreshold) {
      ...
      resetSystem(); // Reset if Trigger 4 fails
    }
  }
