...
float weightDropThreshold = 100; // Drastic drop threshold (80% of baseline)
...
float tiltThreshold = 60.0;    // Tilt angle threshold (degrees)
...
// Variables
int triggerCheck = 1;  
int alarmRaise = 1;       // Trigger stage (1: Trigger 1, 2: Trigger 2, 3: Trigger 3, 4: Trigger 4)
...
unsigned long trigger3Time = 0; // Time when Trigger 2 was detected
...
void loop() {
  ...
  if (triggerCheck == 3) {
    ...
    if (totalAccel > accelThreshold) {
      trigger3Time = millis();
      baselineTiltAngle = calculateTiltAngle(accelX, accelY, accelZ); // Save baseline tilt angle
      Serial.println("Trigger 3 Activated: Sudden Impact Detected!");
      Serial.print("Baseline Tilt Angle: "); Serial.println(baselineTiltAngle);
      triggerCheck++; // Proceed to Trigger 4
      alarmRaise++;
  ...
  if (triggerCheck == 4) {
   ...
   if (triggerCheck == 5) {
    ...
    if (weightDrop > weightDropThreshold) {
        ...
        Serial.println("Trigger 5 Not Detected: System Resetting.");
        resetSystem();
    }
  }
